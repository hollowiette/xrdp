...
send_message(int code, char *data, int bytes)
{
    ...
    LLOGLN(0, ("send_message code: %d", code));
    LLOGLN(0, ("send_message bytes: %d", bytes));
    
    SET_UINT32(header, 0, bytes);
    SET_UINT32(header, 4, code);
    
    ssize_t send_result = send(g_sck, header, 8, 0);

    LLOGLN(0, ("send_message send_result != 8: %d", send_result));
    if (send_result != 8)
    {
    ...
    send_result = send(g_sck, data, bytes, 0);
    LLOGLN(0, ("send_message send_result != bytes: %d", send_result));
...
get_message(int *code, char *data, int *bytes)
{
    ...
    LLOGLN(0, ("get_message:"));
    max = g_sck + 1;
    while (1)
    {
        LLOGLN(0, ("get_message: loop"));
        time.tv_sec = 1;
        time.tv_usec = 0;
        FD_ZERO(&rd_set);
        ...
        if (error == 1)
        {
            ...
            if (error == 1)
            {
                ...
                if (recv_rv == 8)
                {
                    ...
                    {
                        LLOGLN(0, ("get_message: lcode %d *code %d",
                                    lcode, *code));
                    }
                }
                else if (recv_rv == 0)
                ...
                {
                    LLOGLN(0, ("get_message: recv_rv %d", recv_rv));
                }
            }
            else
            {
                LLOGLN(0, ("get_message: select return %d", error));
            }
            pthread_mutex_unlock(&g_mutex);
            LLOGLN(0, ("get message loop end success"));
            usleep(1000);
        }
    }
    ...
    *code = GET_UINT32(header, 4);
    ...
    {
        pthread_mutex_unlock(&g_mutex);
        LLOGLN(0, ("mutex unlock bytes > max_bytes"));
        return 1;
    }
    if (recv(g_sck, data, *bytes, 0) != *bytes)
    {
        pthread_mutex_unlock(&g_mutex);
        LLOGLN(0, ("mutex unlock recv != bytes"));
        return 1;
    }
    pthread_mutex_unlock(&g_mutex);
    LLOGLN(0, ("get message success"));
    return 0;
}
                      ...
                      LPSCARDCONTEXT phContext)
{
    ...
    LLOGLN(0, ("SCardEstablishContext: got context 0x%8.8x", (int)context));
    *phContext = context;
                     ...
                     LPSCARD_READERSTATE rgReaderStates, DWORD cReaders)
{
    ...
    LLOGLN(0, ("  dwTimeout %d cReaders %d", (int)dwTimeout, (int)cReaders));
    if (g_sck == -1)
    {
                 ...
                 LPDWORD pcchReaders)
{
    ...
    LLOGLN(0, ("SCardListReaders: mszGroups %s", mszGroups));
    LLOGLN(0, ("SCardListReaders: *pcchReaders %d", (int)*pcchReaders));
    if (g_sck == -1)
    {
    ...
    LLOGLN(0, ("SCardListReaders: mszReaders %p pcchReaders %p num_readers %d",
    ...
    if (*pcchReaders == SCARD_AUTOALLOCATE)
    {
        ...
        if ((readers_list_out = (char *)malloc(GET_UINT32(msg, 0))) == NULL)
        {
            return SCARD_E_NO_MEMORY;
        }
        LLOGLN(0, ("SCardListReaders END: SCARD_AUTOALLOCATE"));
        *(char **)mszReaders = readers_list_out; // Pass pointer to  user
        mszReaders = readers_list_out; // Use pointer ourselves
        *pcchReaders = num_readers;
    }

    //LLOGLN(0, ("SCardListReaders: new pcchReaders %d", pcchReaders));
    LLOGLN(0, ("SCardListReaders END: reader_names_index %d", reader_names_index));

    LLOGLN(0, ("SCardListReaders END: mszReaders %p pcchReaders %p num_readers %d",
                mszReaders, pcchReaders, num_readers));
                 ...
                 LPDWORD pcchReaders)
{
    ...
    LLOGLN(0, ("SCardListReaders: hContext 0x%8.8x", (int) hContext));
    ...
    */
    ...
    {
        memcpy(reader, msg + offset, 100);
        LLOGLN(0, ("SCardListReaders: reader %s", reader));
        bytes = strlen(reader);
        memcpy(reader_names + reader_names_index, reader, bytes);
        reader_names_index += bytes;
        reader_names[reader_names_index] = 0;
    ...
    {
        char *readers_list_out;
        if ((readers_list_out = (char *)malloc(num_readers)) == NULL)
        {
            return SCARD_E_NO_MEMORY;
        }
        LLOGLN(0, ("SCardListReaders END: SCARD_AUTOALLOCATE"));
