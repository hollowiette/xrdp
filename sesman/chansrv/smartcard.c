                       ...
                       char *groups, int cchReaders, int wide)
{
    ...
    if (bytes_groups > 0)
    {
        ...
        //out_uint32_le(s, bytes_groups);
        /*
        unsigned int over = (unsigned int)(s->p - s->data) % 4;
        if (over != 0)
        {
            out_uint8s(s, 4 - over);
        }
                                ...
                                tui32 IoStatus)
{
    ...
    LOG(LOG_LEVEL_INFO, "entered");
    /* sanity check */
    if ((DeviceId != irp->DeviceId) || (CompletionId != irp->CompletionId))
    {
        LOG(LOG_LEVEL_INFO, "DeviceId/CompletionId do not match those in IRP");
        return;
    }
    /* get OutputBufferLen */
    xstream_rd_u32_le(s, len);
