                                ...
                                tui32 app_context)
{
    ...
    {
        LOG(LOG_LEVEL_INFO, "get_pcsc_context_by_app_context: %d", index);
...
scard_process_establish_context(struct trans *con, struct stream *in_s)
{
    ...
    LOG(LOG_LEVEL_INFO, "scard_process_establish_context:");
    uds_client = (struct pcsc_uds_client *) (con->callback_data);
    in_uint32_le(in_s, dwScope);
    LOG(LOG_LEVEL_INFO, "scard_process_establish_context: dwScope 0x%8.8x", dwScope);
                                        ...
                                        int len, int status)
{
    ...
    LOG(LOG_LEVEL_INFO, "scard_function_establish_context_return:");
    LOG(LOG_LEVEL_INFO, "  status 0x%8.8x", status);
    ...
    if (status == 0)
    {
        ...
        if (context_bytes > 16)
        {
            LOG(LOG_LEVEL_INFO, "scard_function_establish_context_return: opps "
...
scard_process_release_context(struct trans *con, struct stream *in_s)
{
    ...
    LOG(LOG_LEVEL_INFO, "scard_process_release_context:");
    uds_client = (struct pcsc_uds_client *) (con->callback_data);
    in_uint32_le(in_s, hContext);
    LOG(LOG_LEVEL_INFO, "scard_process_release_context: hContext 0x%8.8x", hContext);
    user_data = (void *) (tintptr) (uds_client->uds_client_id);
                                      ...
                                      int len, int status)
{
    ...
    LOG(LOG_LEVEL_INFO, "scard_function_release_context_return:");
    LOG(LOG_LEVEL_INFO, "  status 0x%8.8x", status);
    uds_client_id = (int) (tintptr) user_data;
    uds_client = (struct pcsc_uds_client *)
                 get_uds_client_by_id(uds_client_id);
...
scard_process_list_readers(struct trans *con, struct stream *in_s)
{
    ...
    LOG(LOG_LEVEL_INFO, "scard_process_list_readers:");

    LOG(LOG_LEVEL_INFO, "scard_process_list_readers: in_s->p = %p", in_s->p);
    LOG(LOG_LEVEL_INFO, "scard_process_list_readers: in_s->end = %p", in_s->end);
    LOG(LOG_LEVEL_INFO, "scard_process_list_readers: in_s->data = %p", in_s->data);
    LOG(LOG_LEVEL_INFO, "scard_process_list_readers: in_s->size = %d", in_s->size);
    LOG(LOG_LEVEL_INFO, "scard_process_list_readers: in_s->pad0 = %d", in_s->pad0);
    LOG(LOG_LEVEL_INFO, "scard_process_list_readers: in_s->iso_hdr = %p", in_s->iso_hdr);
    LOG(LOG_LEVEL_INFO, "scard_process_list_readers: in_s->mcs_hdr = %p", in_s->mcs_hdr);
    LOG(LOG_LEVEL_INFO, "scard_process_list_readers: in_s->sec_hdr = %p", in_s->sec_hdr);
    LOG(LOG_LEVEL_INFO, "scard_process_list_readers: in_s->rdp_hdr = %p", in_s->rdp_hdr);
    LOG(LOG_LEVEL_INFO, "scard_process_list_readers: in_s->channel_hdr = %p", in_s->channel_hdr);
    LOG(LOG_LEVEL_INFO, "scard_process_list_readers: in_s->next_packet = %p", in_s->next_packet);
    LOG(LOG_LEVEL_INFO, "scard_process_list_readers: in_s->next = %p", in_s->next);
    LOG(LOG_LEVEL_INFO, "scard_process_list_readers: in_s->source = %p", in_s->source);
    ...
    {
        LOG(LOG_LEVEL_INFO, "scard_process_list_readers: Unreasonable string length %u",
            bytes_groups);
        return 1;
    }
    ...
    LOG(LOG_LEVEL_INFO, "scard_process_list_readers: hContext 0x%8.8x cchReaders %d",
              hContext, cchReaders);
    lcontext = get_pcsc_context_by_app_context(uds_client, hContext);
    if (lcontext == 0)
    {
                                   ...
                                   int len, int status)
{
    ...
    LOG(LOG_LEVEL_INFO, "scard_function_list_readers_return:");
    LOG(LOG_LEVEL_INFO, "  status 0x%8.8x", status);
